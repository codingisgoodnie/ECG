...
from delete_fig import delete_all_figure
...
def ecg_PVC_Selection(signal, fs):
    ...
    test4 = []
    cnt = 0 # 记录保存了多少张图片的变量
    max_figure_num = 100 # 记录超过max_figure_time的图片就退出
    dir_path = r"E:\dl_data\figure" # 生成图片太多，需要删除的图片所在文件夹地址
    if len(abnormals)>0:
        ...
        if len(test4)>0:
            ...
            for i in range(len(temp)):
                ...
                myTicks = int((end-start) * amplifier / 30)
